...
correlation_score_merged_spearman <- function(up_signature, down_signature, rank_matrix, compound_name) {
    n = 1
    connectivity_score_vec <- c()
    gene_rank_bing <- rank_matrix
    for (i in 1:ncol(gene_rank_bing)) {
        ...
        merged_signature <- c(up_signature, down_signature)
        rank_v <- match(merged_signature, pert_vec)
        tmp_s <- cor((1:length(merged_signature))/length(merged_signature), rank_v/nrow(gene_rank_bing), method = "spearman")
        connectivity_score_vec <- c(connectivity_score_vec, tmp_s) 
...
weighted_correlation_score_merged <- function(up_signature, down_signature, up_logfc, down_logfc, compound_name) {
    n = 1
    connectivity_score_vec <- c()
    gene_rank_bing <- rank_matrix
    for (i in 1:ncol(gene_rank_bing)) {
